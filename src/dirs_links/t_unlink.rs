...
pub fn main(args: &[String]) -> ! {
    ...
    let path_s = unsafe { CString::new(args[1].as_str()).expect("Failed to create CString") };
    ...
    let cmd_s = unsafe { CString::new(cmd_str.as_str()).expect("Failed to create CString") };
    unsafe {
	system(cmd_s.as_ptr());
    };
