...
use libc::{itimerspec, CLOCK_REALTIME, timerfd_create, timerfd_settime, clock_gettime, CLOCK_MONOTONIC, read, exit, EXIT_SUCCESS, timespec, ssize_t};
...
fn timespec_from_str(s: &str) -> Result<timespec, &'static str> {
	...
	Ok(timespec { tv_sec: seconds, tv_nsec: nanos })
    }
}
...
fn itimerspec_from_str(s: &str) -> Result<itimerspec, &'static str> {
	...
	Ok(itimerspec { it_value: value, it_interval: interval })
    }
}

pub fn main(args: &[String]) -> ! {
    if args.len() < 2 || (args.len() > 1 && args[1].as_str() == "--help") {
	usage_err(&format!("{} secs[/nsecs][:int-secs[/int-nsecs]] [max-exp]", args[0]));
    }
	...
	let s = unsafe { read(fd, &mut num_exp as *mut u64 as *mut c_void, mem::size_of::<u64>()) };
	...
	println!("{}.{}: expirations read: {}, total={}",
		 secs,
